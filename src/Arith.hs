...
toPostfix (BinOp op left right) = toPostfix left ++ " " ++ toPostfix right ++ " " ++ show op
...
foldTree :: [AST] -> String -> (Maybe [AST])
foldTree stack str = case parseNum str of
    Just (treeNum, rest) -> Just (treeNum:stack)
    Nothing -> do
        (op, rest) <- parseOp str
        (l:r:stack') <- Just stack
        return ((BinOp op r l):stack')

fromPostfix :: String -> Maybe AST
fromPostfix s = do
    let input = Sum.splitOn ' ' s
    [tree] <- foldM foldTree [] input
    return tree
