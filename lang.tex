...
\pagestyle{fancy}
 ...
 Давайте сконструируем язык \textbf{L} через \textbf{обратную польскую запись}.\\
 \\
 ...
 Нетерминальный алфавит $N =\{s_0, E_0, V_0, D_0,$\textvisiblespace$  \}$, где $s_0$ - стартовое состояние, $E_0$ - это Expr, $V_0$ - это Var и Ident (в данном случае это одно и тоже), $D_0$ - это числа, \textvisiblespace - все пробельные символы \\
 \\
 В нашем языке есть ключевые слова \textbf{If, While, Assign, Read, Write и Seq} \\
 Для каждого из них введём правило: \\ 
 \begin{itemize}
 	\item $s_0 \to E_0 $\textvisiblespace$ s_0 $\textvisiblespace$ s_0 $\textvisiblespace$ If$ \\
 	(Порядок веток прямой, соответственно если условие $E_0$ выполнится, то будет исполнятся первый $s_0$, иначе второй) 
 	\item $s_0 \to E_0 $\textvisiblespace$ s_0 $\textvisiblespace$ While$ 
 	\item $s_0 \to V_0 $\textvisiblespace$ E_0 $\textvisiblespace$ Assign$ 
 	\item $s_0 \to V_0 $\textvisiblespace$ Read$ 
 	\item $s_0 \to E_0 $\textvisiblespace$ Write$ 
 	\item $s_0 \to s_0 $\textvisiblespace$ s_0 $\textvisiblespace$ Seq$ \\
...
\vspace{\baselineskip}
 ...
 \begin{itemize}
	\item $E_0 \to E_0 $\textvisiblespace$ E_0 $\textvisiblespace$ -$ \\
...
\end{itemize}
  ...
  Правила для унарных операторов (пока что только унарный минус): \\
  
  \begin{itemize}
 	\item $E_0 \to E_0 $\textvisiblespace$ - -$
 \end{itemize}
